...
> 我把邮箱状态修改视为危险工具。模型和用户都可以提出动作，但系统不会直接执行，必须经过服务端权限检查和用户审批。

## 2.1 API 和开发工具安全边界

服务端不是开放式远程 API。设置 `WISPERA_AUTH_TOKEN` 后，除 `/health` 外的接口都需要 bearer token。Windows client 会从环境变量读取同一个 token 并自动附加请求头。
...
运行时 provider 通过 `server/app/provider_factory.py` 装配。当前 `WISPERA_EMAIL_PROVIDER` 只接受空值或 `mock`，未知值会直接报错。这是刻意保守的设计：真实 provider 未完成前，不让拼写错误或未支持配置静默落到 mock，避免测试和演示时误判当前接入状态。
...
> 我没有直接让 LLM 控制邮箱，而是先让 LLM 在固定 mock 数据上做 shadow classification，并和 deterministic baseline 共用同一套 metrics。这样能把 prompt/schema/model 问题和真实邮箱 provider 问题拆开调试。
...
报告只包含 metrics、错误数、mismatch 摘要和评估说明，不保存完整邮件正文、API key 或长 rows。这个功能用于面试展示和后续回归对比，不替代自动化测试。

报告导出工具只允许写入 `docs/test-logs/`。它不能覆盖源码、README、配置或其他工作区文件。Mock eval 工具永远使用 `MockEmailProvider`，即使后续 active provider 换成 Outlook，也不会误扫真实邮箱。
...
这个边界仍不是分布式调度系统。未来如果部署多进程后台 worker，还需要明确 leader election、job lease 或外部队列。但对当前 FastAPI demo 和本地 Windows 客户端来说，已经覆盖了最主要的重复扫描风险。
...
因此 Phase 6A 只做 Outlook / Microsoft Graph read-only。真实写操作、Gmail provider、后台 service 和复杂 UI 都应推迟。

Phase 6A 的实现边界：

- provider 层把 Graph message 标准化成 `EmailMessage`。
- 上层 tools、classifier、scheduler、eval 尽量复用。
- trace 中避免记录过长正文。
- 不执行 archive、mark read、star、draft、send、delete。
