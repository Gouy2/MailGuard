...
- Inbox Cleaner dry-run：只有 enabled clean rule 或 confirmed sender/domain memory 且未被 protected guard / protect rule 拦截时才能进入 auto_eligible；不创建 proposal、不写 audit、不调用 archive。
- Clean Rule teaching：自然语言偏好只生成 proposed clean/protect rules 和 impact preview；approve/disable 只改本地规则状态，不修改邮箱。
- Clean Run execution：`clean-run` 默认不执行；`--yes` 只执行 auto_eligible，并写 clean audit events。
...
默认状态写入 `server/data/mailguard_state.db`；需要隔离测试时再设置 `MAILGUARD_STATE_DB=/private/tmp/mailguard-proposal-test.db`。
...
- `teach` 默认使用 heuristic parser；`--llm openai` 只用于 parser，不授权写操作。
- rule 初始状态是 `proposed`，必须 `rule approve` 后才会影响 `clean-preview`。
- `protect` rule 和 `protected` guard 优先于 `archive` rule。
...
默认状态写入 `server/data/mailguard_state.db`；需要隔离真实审核状态时再设置 `MAILGUARD_STATE_DB=/private/tmp/mailguard-review-test.db`。
