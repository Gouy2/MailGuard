...
`--live` 会调用真实 LLM API，但仍强制 mock provider，不读取真实 QQ/Foxmail。
...
1. 在 mock provider 上跑 live LLM agent smoke。
2. 在真实 QQ/Foxmail 上做 agent read-only 测试。
3. 用专门测试邮件做真实 pending write 测试，并通过 `agent_cli.py` 查询 pending/trace。
4. 基于真实标签文件迭代分类质量。
5. CLI 闭环稳定后再做最小 UI。
