...
- `server/app/cleaner/`：Inbox Cleaner dry-run workflow；复用 archive plan，只生成 `auto_eligible` preview artifact，不创建 proposal、不执行邮箱写操作。
...
`server/app/cleaner/` 是当前主线的只读 dry-run workflow，用来验证未来自动归档前的 `auto_eligible` 边界。

```text
email_cli clean-preview
-> cleaner.run_clean_preview
-> archive.build_archive_plan
-> confirmed sender/domain memory gate
-> clean preview artifact
```

第一版只允许 approved `archive_sender` / `archive_domain` memory 把低风险邮件列为 `auto_eligible`。`protected` 永远优先；category memory、LLM shadow 结果和单纯严格规则都不能授权 auto-eligible。

Clean preview artifact 保存 counts、auto_eligible、protected、candidate 和 no_action，并明确 `mailbox_mutation=false`、`proposal_mutation=false`、`llm_authorization=false`。它不创建 `ActionProposal`，不写 audit log，不调用 provider 的写操作。
