...
MailGuard 当前是 headless-first 的邮件管理 Agent 内核，并新增本地 Agent Console 作为开发调试控制台。

```text
React Console / FastAPI / CLI
  -> AgentRuntime
     -> ToolRegistry
     -> MemoryStore
     -> TraceLogger
...
- `console/`：React + Vite Agent Console；展示 chat、实时 trace、pending approval 和 Inbox Cleaner teach/rules/preview。
...
React Console、CLI、API/SSE 和后台任务都应复用 `cleaner/`、`archive_shadow_workflow.py`、`memory_workflow.py`、`archive/` core 和 `email_proposals.py`，而不是复制缓存、artifact、latency、label、proposal 或 cleaner 状态逻辑。Console 只做参数转换、状态展示和用户交互，不拥有业务决策。
...
Chat SSE 会输出 `status`、`token`、`trace`、`done`、`error`。`trace` event 使用与 `TraceLogger` JSONL 一致的脱敏 record，用于 Console 实时展示 observable chain。产品 trace 不展示模型原始 chain-of-thought，只展示 LLM/tool/approval/error 等可观察事件。
...
React Console 第一版通过 FastAPI 调用 `cleaner` workflow：

```text
POST /cleaner/teach
GET /cleaner/rules
POST /cleaner/rules/{rule_id}/approve|disable
POST /cleaner/preview
GET /cleaner/policy
GET /cleaner/audit
```

这些接口同时挂载在 `/api/*`，供 Vite dev proxy 和 FastAPI `/console` 静态挂载共用；无前缀接口继续保留给 CLI smoke、已有测试和直接 API 调试。

Chat agent 同时暴露 `email_clean_teach`、`email_clean_rules`、`email_clean_preview`、`email_clean_approve_rule`、`email_clean_disable_rule`、`email_clean_policy` 和 `email_clean_audit`。其中 `email_clean_approve_rule` 是 dangerous tool，因为批准 rule 会影响未来自动归档授权。
