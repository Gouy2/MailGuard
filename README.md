...
- 真实 QQ/Foxmail agent read-only smoke
...
`--live` 会调用真实 LLM API，但仍强制使用 mock provider，不读取真实邮箱。`--real-readonly` 使用当前 `.env` 的 QQ/Foxmail provider，但运行在 `agent_readonly` 模式，不暴露邮箱写工具。`--real-pending-write` 会验证真实 provider 下写工具只进入 pending，并立即 reject，不 approve。
