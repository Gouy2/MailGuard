...
`mailguard clean-run` 默认仍不修改邮箱，只显示会执行哪些 `auto_eligible`；只有加 `--yes` 才会调用 provider archive，并把 started/succeeded/failed 写入 clean audit。真实邮箱执行前应先做只读 preview 并确认规则范围。
